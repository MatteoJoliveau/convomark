...
import {CollectionRepository, BookmarkRepository, TypeORMBindings} from '../../typeorm';
...
export class BookmarkListWidgetProvider implements Provider<Widget>, Loggable {
    ...
    private sendReply(
        ...
        const bookmarkList = boookmarks.reduce((text, {name, messageLink}) => {
            const link = name ? `<a href="${messageLink}">${name}</a> (${messageLink})` : messageLink;
            return `${text}${link}\n\n`;
        }, '');
        const message = `
...
${bookmarkList}
    ...
    private generateKeyboard(widget: Widget, {total, offset, collectionId: id, ctx: {i18n}}: KeyboardGenerationParams) {
        this.logger.debug({offset, limit: LIMIT, total}, 'Generating keyboard for %d bookmarks out of %d', LIMIT, total);
