...
export class CollectionRepository extends DefaultCrudRepository<Collection,
    typeof Collection.prototype.id,
    CollectionRelations> {
    public readonly bookmarkCollections: HasManyRepositoryFactory<BookmarkCollection,
        typeof Collection.prototype.id>;
    ...
    async update(
        ...
        const username = user.username || user.id;
        entity.slug = slug(`${username} ${entity.title}`, {lower: true});
        entity.shortId = entity.shortId || ((e) => {
            const hash = createHash('SHA256');
            hash.update(e.id);
    ...
    bookmarks(
        ...
        return async (joinFilter) => {
            ...
            const joins = await this.bookmarkCollections(collection.id).find(joinFilter);
